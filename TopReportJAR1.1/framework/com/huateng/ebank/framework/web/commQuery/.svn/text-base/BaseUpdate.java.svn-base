...
public abstract class BaseUpdate extends _CallUpdate {
	...
	@Override
	public Map<String, String> preProcess(Map<String, String> param,
	...
	public UpdateReturnBean process(
		...
		try {
			try{
				databusId = (String) ((multiUpdateResultBean.getParamMap())
						.get("databusId"));
				this.multiUpdateResultBean = multiUpdateResultBean;
				//init(request);
				globalInfo = GlobalInfo.getCurrentInstanceWithoutException();
				UpdateReturnBean update = super.process(multiUpdateResultBean,
						request, response);
				if (globalInfo != null) {
...
//			globalInfo.setFuncCode(funcCode);
	 ...
	 * @throws AppException
	 */
	public static void mapToObject2(Object object, Map map)
			throws AppException {
		try {
	...
	protected boolean setSessionObject(String attrName, Object value) {
		return SessionManager.getInstance().setSessionObject(httpReq, attrName,
				value);
	}
	...
	protected boolean removeSessionObject(String attrName) {
		return SessionManager.getInstance().removeSessionObject(httpReq,
				attrName);
	}
}
