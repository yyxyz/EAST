...
public abstract class BaseGetter extends _CallGetter {
	...
	/* (non-Javadoc)
	 * @see com.huateng.commquery.process.call._CallGetter#process(java.util.Map, javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse)
	 */
	@Override
	public ResultParamBean process(Map param,
		...
		try{
					...
					if (param.containsKey(ReportConstant.SAVE_QUERY_LOG_SIGN)) {
						...
						if (isSaveLog!=null && isSaveLog.equals("1") && "1".equals(ginfo.getSaveQueryLog())) {//记录日志
							...
							if (busiName!=null && busiName.trim().length()>0) {
								ginfo.addBizLog("Updater.log", new String[] { ginfo.getTlrno(), ginfo.getBrno(), busiName });
							}
						}
					}
				}
			...
			throw new AppException("", cex.getKey(),
					ErrorCodeUtil.convertErrorMessage(log, cex), cex);
			...
			throw new AppException(Module.SYSTEM_MODULE,
					Rescode.DEFAULT_RESCODE,new Exception(te));
		}
		finally{
	 ...
	 */
	@Override
	public Map preProcess(Map param,
	    		...
	    		if(txnLogFlag!=null && txnLogFlag.equals("true")&&funcLogFlag.equals("true")){
	    			...
	    			if(list!=null&&list.size()!=0){
	    				...
	    				for(int i= 0;i<list.size();i++){
	    					//取得操作员的流水号
	    					try {
	    						String sequence = ((GetBizLogNoGentator) GeneratorFactory.getGenerator("GetBizLogNoGentator")).gen(null);
	    						//String sequence=getBizLogNoGentator.gen(null);
	    						//Integer  sequence = GeneratorFactory.getDialectSeq().getSeqNo("SEQ_BIZ_LOG_TLSRNO");
	    						//将操作员的流水号写入文件日志
	    						globalInfo.setBaseTlsrno(sequence);
	    						logger.printNoLevel("操作员的流水号:"+sequence,logger.lever_txnLog,true);
	    						BizLogThreadPoolExecutor.addTask(globalInfo,globalInfo.getBaseTlsrno(),(String) list.get(i),txnLogFlag,uuId,oprTxnCd);
	    			...
	    			if(stringLog!=null&&!stringLog.equals("")){
	    				try {
    						...
    						globalInfo.setBaseTlsrno(sequence);
    						logger.printNoLevel("操作员的流水号:"+sequence,logger.lever_txnLog,true);
    						BizLogThreadPoolExecutor.addTask(globalInfo,globalInfo.getBaseTlsrno(),stringLog,txnLogFlag,uuId,oprTxnCd);
    					} catch (AppException e) {
    						// TODO Auto-generated catch block
    						e.printStackTrace();
    					}
...
//			globalInfo.setFuncCode(funcCode);
	...
	protected GlobalInfo checkGlobalInfo(HttpServletRequest request)
		...
		/** add jiang@20091102 */
		/*
		if(UserMgrService.getInstance().isKicked(globalInfo,request.getSession()))
		{
