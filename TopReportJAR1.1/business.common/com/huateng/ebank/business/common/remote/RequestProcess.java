...
public class RequestProcess {
	...
	public static boolean processBack(HttpServletRequest request,
	...
	public static GlobalInfo initGlobalInfo(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		if(value_map != null && !value_map.trim().equals("")){
			...
			if(map !=null){
				...
				else go2ErrorPage(request,response);
		}

		globalInfo = (GlobalInfo) session
	...
	public static Object getValueFromRequest(HttpServletRequest request,
			String key) {
		...
		if (value != null) {
			...
			if (value == null) {
				Object value_map = request
						.getAttribute(TransferConstant.SEND_VALUE_MAP);
				if (value_map != null)
	...
	private static void go2ErrorPage(HttpServletRequest req,
