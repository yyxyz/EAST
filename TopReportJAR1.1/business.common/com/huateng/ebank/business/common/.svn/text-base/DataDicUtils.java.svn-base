...
public class DataDicUtils {
	...
	/**
	 * <P>initDataDic:(这里用一句话描述这个方法的作用)
	 * <P>适用条件: (这里描述这个方法适用条件 – 可选)
	...
	public static void initDataDic(Locale locale)throws CommonException{
		try{
		    ...
		    while(it.hasNext()){
		    	...
		    	if (locale != null) {
		    		String dataNameInter = mess.getMessage(locale, dataDic.getDataTypeNo()+"."+dataDic.getDataNo().trim());
		    	...
		    	SystemDictionaryUnit.addRecord(
		    			tblNm,
		    			String.valueOf(dataDic.getDataTypeNo()),
		    			dataDic.getDataNo().trim(),
		    			dataName);
