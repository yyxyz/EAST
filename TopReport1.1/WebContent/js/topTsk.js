...
var detail = {
		showUodoTaskDetail:function(type,sta,rcdPk){
			...
			if(type=="100199"){
				...
				showWin("角色管理详细信息",_application_root+"/fpages/system/ftl/RoleFuncMngWithEdit.ftl?id="+rcdPk+"&flag=0&st="+st+"&type=" + type,null,null,window);
			}else if(type=="100399"){
				showWin("用户详细信息",_application_root+"/fpages/regonization/ftl/OperMngRoleCompare.ftl?id=" + rcdPk + "&st=" + st + "&flag=0&type=" + type,"","",window);
				//loadPageWindows("partWin", "用户管理详细信息","#", paramMap, "winZone");
			}else if(type=="100599"){
				showWin("工作日期详细信息",_application_root+"/fpages/system/ftl/BiWorkDateDetail.ftl?id="+rcdPk+"&flag=0&st="+sta+"&type="+type,"","",window);
			}else if(type=="100799"){
...
var Compdetail = {
		showCompTaskDetail:function(type,sta,rcdPk,tskId){
			...
			if(type=="100199"){
				...
				showWin("角色管理详细信息",_application_root+"/fpages/system/ftl/RoleFuncMngWithEdit.ftl?id="+rcdPk+"&flag=1&st="+st+"&tskId="+tskId,null,null,window);
			}else if(type=="100399"){
				//loadPageWindows("partWin", "用户管理详细信息","/fpages/regonization/ftl/OperMngRoleInfoCompare.ftl", paramMap, "winZone");
				showWin("用户详细信息",_application_root+"/fpages/regonization/ftl/OperMngRoleCompare.ftl?id=" + rcdPk + "&st=" + st + "&flag=1&type=" + type+"&tskId=" + tskId,"","",window);
			}else if(type=="100599"){
				showWin("工作日期详细信息",_application_root+"/fpages/system/ftl/BiWorkDateDetail.ftl?id="+rcdPk+"&flag=1&st="+sta+"&type="+type+"&tskId="+tskId,"","",window);
			}else if(type=="100799"){
				loadPageWindows("partWin", "系统参数详细信息","/fpages/system/ftl/SysParamsEntryDetail.ftl", paramMap, "winZone");
			}else if(type=="100899"){
				loadPageWindows("partWin", "安全参数详细信息","/fpages/system/ftl/SysParamsSecDetail.ftl", paramMap, "winZone");
