...
strFrame+='        <td colspan=3 align=right Author=meizz><span onclick=parent.closeDateLayer() style="font-size:12px;cursor: hand"';
...
function document.onclick(){
  with(window.event){
    if (srcElement.getAttribute("Author")==null && srcElement != outObject && srcElement != outButton)	closeDateLayer();
  }
}
...
function meizzDayClick(n,ex){
  ...
  closeDateLayer();
    ...
    closeDateLayer(); alert("您所要输出的控件对象并不存在！");
  }
}
