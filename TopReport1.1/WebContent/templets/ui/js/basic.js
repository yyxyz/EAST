...
function formatDateTime(date, mode){
	...
	if (typeof(date)=="object" && !isNaN(date)){
		...
		switch (mode){
		/*added by wangpeng 20091126  BMS-2274 ���predate��postdate���������֧��  begin*/
		    case "predate":;
		    case "postdate":;
		/*added by wangpeng 20091126 BMS-2274  ���predate��postdate���������֧��  end*/
			case "date":{
...
/**modified by wangpeng CSYS-14 20100910 �޸�������ʽչʾ���� begin*/
//�����ֽ�����ǧλ�ָ�
function formatCurrency(orgValue){
	...
	var v_return_value;
	var digit = orgValue.indexOf("."); // ȡ��С����λ��
	var int = orgValue.substr(0,digit); // ȡ��С���е������
	var i;
	var mag = new Array();
	var word;
	if (orgValue.indexOf(".") == -1) { // ����ʱ
		i = orgValue.length; // ����ĸ���
		while(i > 0) {
			word = orgValue.substring(i,i-3); // ÿ��3λ��ȡһ������
			i-= 3;
...
function cast(value, _to) {
	...
	switch (_to) {
		...
		case "timestamp": {
			if(_from == "string") {
				var reg1 = new RegExp("^\\d{4}([-/])\\d{1,2}\\1\\d{1,2} \\d{2}:\\d{2}:\\d{2}$");//19
